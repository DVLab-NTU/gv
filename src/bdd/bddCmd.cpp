...
/*static bool valid()
{
  ...
  if (!handler)
    ...
    gvMsg(MSG_ERR) << "Current Network is NOT an AIG Ntk (try \"blast ntk\" first)!!" << endl; else return true; return false;
} */
...
// {
//   gvMsg(GV_MSG_IFO) << "Usage: BEXist <(size_t level)> <(string varName)> <(string
// bddName)>\n";
// }
...
BSetOrderCmd::exec(const string& option)
{
  ...
  file = true;
  setBdd = gvNtkMgr->setBddOrder(true);
  if (!setBdd) gvMsg(GV_MSG_ERR) << "Set BDD Variable Order Failed !!" << endl; 
...
//   if (myStrNCmp("-All", options[0], 2) == 0) ntk->buildNtkBdd();
...
//     int num = 0;
//     V3NetId netId;
//     if(!myStr2Int(options[1], num) || (num < 0)) return
// GVCmdExec::errorOption(GV_CMD_OPT_ILLEGAL, options[1]); if(isNet) {
//       if((unsigned)num >= ntk->getNetSize()) {
//         gvMsg(MSG_ERR) << "Net with Id " << num << " does NOT Exist in Current
//         Ntk
...
//         gvMsg(MSG_ERR) << "Output with Index " << num << " does NOT Exist in
// Current Ntk !!" << endl; return GVCmdExec::errorOption(GV_CMD_OPT_ILLEGAL,
// options[1]);
//       }
...
// {
//   gvMsg(GV_MSG_IFO) << "Usage: BConstruct <-Netid <netId> | -Output <outputIndex>
//   |
// -All>" << endl;
// }
