...
public:
    ...
    // operator overload
    template <class T>
    friend GVMsg& operator<<(GVMsg& m, const T& s) {
...
enum GVMsgType { GV_MSG_LOG = 0, GV_MSG_ERR = 1, GV_MSG_WAR = 2, GV_MSG_DBG = 3, GV_MSG_IFO = 4 };
...
public:
    GVMsgMgr() {
        Msgs.clear();
        MsgTypeString.clear();
        Msgs.push_back(new GVMsg(cout));
        MsgTypeString.push_back("");  // 0 : CmdLogMsg
        Msgs.push_back(new GVMsg(cout));
        MsgTypeString.push_back("[ERROR]: ");  // 1 : ErrorMsg
        Msgs.push_back(new GVMsg(cout));
        MsgTypeString.push_back("[WARNING]: ");  // 2 : WarningMsg
        Msgs.push_back(new GVMsg(cout));
        MsgTypeString.push_back("");  // 3 : DebugMsg
        Msgs.push_back(new GVMsg(cout));
        MsgTypeString.push_back("");  // 4 : InfoMsg
    ...
    void stopAllOutFile() {
        if (isAllOutFileON()) GVMsg::_allout.close();
    }
    void startAllDefault() {
        for (unsigned i = 1; i < 5; ++i) Msgs[i]->startDefault();
    }
    void stopAllDefault() {
        for (unsigned i = 1; i < 5; ++i) Msgs[i]->stopDefault();
    }
...
static inline const int gvCallEmptyVirtual(const string& functionName) {
    gvMsg(GV_MSG_ERR) << "Calling virtual function " << functionName << " !!" << endl;
    return 0;
}
